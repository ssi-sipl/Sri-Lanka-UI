...
export async function POST() {
  ...
  const pythonExe = path.join(pythonDir, ".venv", "bin", "python");

  const pythonFile = path.join(pythonDir, "capture_faces.py");

  const manager = processManager.registerFace;
  if (manager.process) {
    return Response.json({
      success: false,
      message: "Register Face is already running.",
    });
  }
